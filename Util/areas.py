...
def hallar_contornos_areas(self, frame, masking):
    ...
    if len(last_areas) == len(areas):        # if exist(True) in list(  if abs(valor)>constante*1.5 => valor = resta(last_areas, areas)  )
        hay_movimiento_segun_cambio_areas = True if True in [True if i>constante_cambio_area*1.5 else False for i in [abs(i) for i in [e1 - e2 for e1, e2 in zip(last_areas,areas)]]] else False
    else:
