...
def Manitor_aleatorio():
    return random.choice(ALL_MACs_MANITOR)
...
TOPIC_BASE = "/" + PROJECT
TOPIC_PUBLISH_MQTT = TOPIC_BASE + "/manitor/" + mac_usar + "/" + CardHolder_aleatorio()
MENSAJE_ENVIAR_MQTT = "{ 'LV' : " + Generar_nivel_bateria_aleatorio() + ", 'HOUR' : " + Leer_HoraActual() + ", 'mac': " + mac_usar + "}"
...
def clic_restart_global():
    topic = TOPIC_BASE + "/manitor/" + mac_usar + "/" + CardHolder_aleatorio()
    msg = "{ 'LV' : " + Generar_nivel_bateria_aleatorio() + ", 'HOUR' : " + Leer_HoraActual() + ", 'mac': " + mac_usar + "}"
